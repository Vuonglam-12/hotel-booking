...
<style>
    ...
    .chatbot-message-bot {
        background: #EAF3FF; color: #3A4A5A;
        border-radius: 12px 12px 12px 4px;
    }
    .chatbot-message-user {
        background: linear-gradient(135deg, #87CEFA, #7BC4F5);
        color: white;
...
{{-- SEARCH HERO — nhiệm vụ của Home: tìm kiếm --}}
<section style="background: linear-gradient(135deg, #87CEFA 0%, #EAF3FF 50%, #FFFFFF 100%);" class="relative overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-4 py-14 text-center">
        ...
        <p class="text-[#5A6A7A] text-base mb-8 max-w-lg mx-auto">Khách sạn trên khắp Việt Nam, giá tốt nhất mỗi ngày</p>
...
<section class="max-w-7xl mx-auto px-4 py-12">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="col-span-3 text-center py-20 text-[#C9D3DD]">
            ...
            <a href="{{ route('home') }}" class="text-[#87CEFA] underline mt-2 inline-block hover:text-[#3A4A5A]">Xem tất cả</a>
        </div>
        @endforelse
    </div>
...
<div class="fixed bottom-6 right-6 z-40">
    <button onclick="toggleChat()"
        class="btn-primary w-14 h-14 rounded-full flex items-center justify-center text-2xl shadow-xl hover:scale-110 transition click-effect"
        title="Chat với AI">
        💬
    </button>
</div>
...
<script>
        ...
        document.getElementById('modalContent').innerHTML = `
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <p class="text-[#5A6A7A] text-sm leading-relaxed">${description || 'Khách sạn sang trọng với dịch vụ đẳng cấp, tiện nghi hiện đại, phù hợp cho kỳ nghỉ của bạn.'}</p>
                </div>
    ...
    async function startChatSession() {
        ...
        if (!token) {
            addChatMessage('bot', 'Vui lòng <a href="/login" class="underline font-semibold">đăng nhập</a> để chat với AI!');
            return;
        }
    ...
    async function toggleWishlist(hotelId, btn) {
        ...
        const data = await res.json();
        const heartFill = data.liked ? 'currentColor' : 'none';
        btn.innerHTML = `<svg class="w-4 h-4" fill="${heartFill}" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>`;
